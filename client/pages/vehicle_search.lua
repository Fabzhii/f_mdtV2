...
RegisterNUICallback('load_vehicle_search', function(data, cb)
    ...
    cb(([[
        <div>
            <sl-tab-group>
                ...
                <sl-tab slot="nav" hx-swap="innerHTML" hx-target="#content" hx-post="/load_vehicle_search" panel="vehicle_search" active>%s</sl-tab>
        ...
        locales['pages']['vehicle_search']['search_header'],
        locales['pages']['vehicle_search']['search_description']
    ))
end)

RegisterNUICallback('search_vehicle', function(data, cb)
    local results = searchVehicle(data.query)
    local search_result = ""

    for k,v in pairs(results) do 
        search_result = search_result .. vehicle_search_vehicle(
            v.plate,
            (v.wanted == 1 and true or false),
            locales['pages']['vehicle_search']['wanted_options'][v.wanted == 1 and true or false],
            locales['pages']['vehicle_search']['owner'], v.owner,
            locales['pages']['vehicle_search']['type'], (GetDisplayNameFromVehicleModel(v.type) == "CARNOTFOUND" and "n/A" or GetDisplayNameFromVehicleModel(v.type)),
            locales['pages']['vehicle_search']['color'], (Config.VehicleColors[v.color] or 'n/A'),
            locales['pages']['vehicle_search']['registered'], locales['pages']['vehicle_search']['registered_options'][v.registered],
            Config.ShowVehicleRegististration
        )
    end 
...
RegisterNUICallback('change_vehicle_wanted', function(data, cb)
    local plate = data.plate
    local wanted = not data.wanted

    local results = changeVehicleWanted(plate, wanted)
